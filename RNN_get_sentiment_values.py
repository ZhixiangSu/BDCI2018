...
print(model.most_similar(["价格"]))
content2vec=[]
for i in range(len(content)):
    content2vec.append([])
    cut=pseg.lcut(content[i])
    for word in cut:
        if(word not in stop_list):
            content2vec[i].append(model[word].tolist())
n_subjects=10
n_values=3
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.i2h = nn.Linear(n_subjects + input_size + hidden_size, hidden_size)
        self.i2o = nn.Linear(n_subjects + input_size + hidden_size, output_size)
...
def randomTrainingTri(l):
    r=random.randint(0,len(l)-1)
    content_tensor=torch.LongTensor(content2vec[r])
    sentiment_value_tensor=torch.zeros(1,n_values)
    sentiment_value_tensor[sentiment_value[r]+1]=1
    subject_tensor=torch.zeros(1,n_subjects)
    subject_tensor[subject_dic[subject[r]]]=1
    return subject_tensor,content_tensor,sentiment_value_tensor
...
for iter in range(1, n_iters + 1):
    output, loss = train(*randomTrainingTri(content))
    total_loss += loss
