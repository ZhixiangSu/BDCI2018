...
rnn=RNN(2*vector_l,n_hidden,n_values)
def TrainingExample(i):
    line=content[i]
    line_tensor=content2tensor(content2vec_subject[i])
...
for iter in range(len(content)):
        line, line_tensor = TrainingExample(iter)
        output= evaluate(line_tensor)
        guess=categoryFromOutput(output)
        if(guess!=0):
            print(guess)
        result_value.append(guess)
raw["sentiment_value"]=result_value
raw["sentiment_value"]=raw["sentiment_value"].astype(int)
raw=raw.drop("content",1)
raw.to_csv("result_tfidf_RNN.csv",encoding="UTF-8",index=False)
